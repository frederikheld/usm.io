...
PageRenderer.prototype.render = function (input, pageMeta = undefined) {
    let fileExt
    if (pageMeta && pageMeta.fileExtension) {
        fileExt = pageMeta.fileExtension
    }

    const mr = new MarkupRenderer(this.renderOptions)
    const output = mr.render(input, fileExt)
